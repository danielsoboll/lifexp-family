...
import type { FamilyPersonalGoal } from '../lib/family/familyPersonalGoals'
import {
  familyCanEditPersonalGoals,
  reorderFamilyPersonalGoals,
  saveFamilyPersonalGoals,
...
type FamilyPersonalGoalsSheetProps = {
  familyId: string
  familyName: string
...
function draftsFromGoals(goals: FamilyPersonalGoal[]): DraftGoal[] {
...
export default function FamilyPersonalGoalsSheet({
  familyId,
  familyName,
  goals,
  canAdmin,
  ...
  onClose,
  onSaved,
}: FamilyPersonalGoalsSheetProps) {
  const canEdit = !readOnly && familyCanEditPersonalGoals({ goals, canAdmin })
  ...
  const moveGoal = async (index: number, direction: -1 | 1) => {
    ...
    if (orderedIds.length === goals.length && goals.length > 0) {
      const { error: reorderError } = await reorderFamilyPersonalGoals(familyId, orderedIds, canAdmin)
  ...
  const handleSave = async () => {
    setLoading(true)
    setError(null)
    const { error: saveError } = await saveFamilyPersonalGoals({
      familyId,
      canAdmin,
        ...
        >
          ...
          <div className="shrink-0">
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">
              {readOnly ? 'Familienziele' : goals.length > 0 ? 'Familienziele' : 'Familienziele anlegen'}
            </h2>
            <p className="mt-1 text-sm text-slate-950 dark:text-slate-300">
              {readOnly
                ? `${familyName} — Ziele sind gesperrt, weil ein Admin XP vergeben hat.`
                : 'Was wünscht sich die Familie als Nächstes? Wähle Text und Symbol — XP legt ein Admin fest.'}
            </p>
          </div>
