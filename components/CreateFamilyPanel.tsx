...
type CreateStep = 'form' | 'install' | 'recovery'
...
export default function CreateFamilyPanel({ onBack }: CreateFamilyPanelProps) {
  ...
  const handleInstallContinue = async (devicePrefs: OnboardingDevicePrefs) => {
    setLoading(true)
    setError(null)
    ...
    const { result, error: createError } = await createFamilyWithMember(familyName, profile, devicePrefs)
  ...
  if (step === 'install') {
    return (
      <div className="space-y-4">
        ...
        <OnboardingPwaStep onContinue={(prefs) => void handleInstallContinue(prefs)} disabled={loading} />
      </div>
    )
  }
