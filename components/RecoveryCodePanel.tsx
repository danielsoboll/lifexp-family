...
type RecoveryCodePanelProps = {
  code: string
  /** Onboarding-Ansicht mit Fortschritts-Hinweis. */
  variant?: 'onboarding' | 'settings'
  recCodeOk?: boolean
  showDoneButton?: boolean
  doneSaving?: boolean
  onDone?: () => void
  /** Erledigt-Hinweis wird von der übergeordneten Sektion gesteuert. */
  ...
  return (
    <div className="flex flex-col gap-3">
      {variant === 'onboarding' ? (
        <div className="rounded-2xl border-2 border-emerald-300/90 bg-emerald-50/95 px-3 py-3 dark:border-emerald-700/55 dark:bg-emerald-950/40">
          ...
          <p className="mt-2 text-sm leading-relaxed text-slate-700 dark:text-slate-300">
            Bitte speichere jetzt deinen Recovery-Code, damit dein Fortschritt niemals verloren geht.
          </p>
        </div>
      ) : null}
      ...
      <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">
        ⚠️ Wenn du dein Handy verlierst oder die App neu installierst, kannst du mit diesem Code
        jederzeit deinen Account wiederherstellen.
      </p>
      ...
      {showDoneButton && !recCodeOk ? (
        <button
          ...
          className="lifexp-pressable-3d w-full rounded-2xl border-2 border-emerald-600 bg-gradient-to-b from-emerald-500 to-emerald-700 px-4 py-3 text-base font-bold text-white hover:from-emerald-400 hover:to-emerald-600 disabled:cursor-not-allowed disabled:opacity-60 dark:border-emerald-500"
        >
