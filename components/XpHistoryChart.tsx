...
import { cetFormatShortDate, cetToday } from '../lib/cetDate'
import type { XpHistoryDay } from '../lib/family/xpHistory'

type XpHistoryChartProps = {
  days: XpHistoryDay[]
  target: number
  max: number
  /** Kompakter für Mitglieder-Zeilen */
  compact?: boolean
  /** Legende nicht im Chart-Rahmen rendern (z. B. darunter außerhalb) */
  hideInlineLegend?: boolean
  /** Y-Achse nicht an Tages-Peak anpassen — max/target definieren die Skala. */
  fixedScale?: boolean
  /** Feste Tick-Schritte (z. B. 5 → 0, 5, 10, …). */
  yAxisStep?: number
  todayKey?: string
}

const DAY_WIDTH = 34
/** Sichtbares Fenster: 7 Tage; ältere per horizontal scroll. */
const VISIBLE_DAY_COUNT = 7
const CHART_HEIGHT_FULL = 212
const CHART_HEIGHT_COMPACT = 176
const PAD_LEFT = 40
const PAD_RIGHT = 16
const PAD_TOP = 18
const PAD_BOTTOM = 34
const DATE_LABEL_OFFSET = 24
const START_LABEL_OFFSET = 12
...
function yAxisTicks(yMax: number, step?: number): { value: number; fraction: number }[] {
  ...
  if (step != null && step > 0) {
    ...
    if (ticks[ticks.length - 1]?.value !== top) {
      ticks.push({ value: top, fraction: 1 })
    }
    return ticks
  }
...
function gradientStops(targetOffsetPct: number, strong: boolean) {
...
export function XpHistoryChartLegend({
  target,
  className = '',
}: {
  target: number
  ...
  return (
    <p className={`px-0.5 text-xs text-slate-950 dark:text-slate-400 ${className}`.trim()}>
      ...
      <span className="font-semibold text-sky-700 dark:text-sky-300">Heute</span> mit blauem Ring.
    </p>
  )
}
...
export default function XpHistoryChart({
  ...
  const padTop = compact ? 14 : PAD_TOP
  ...
  const layout = useMemo(() => {
    ...
    const yMax = fixedScale ? Math.max(max, target, 1) : Math.max(max, target, peakXp, 1)
    ...
    const yForValue = (value: number) =>
      padTop + innerHeight - (Math.min(value, yMax) / yMax) * innerHeight

    const baselineY = padTop + innerHeight
    const targetY = yForValue(target)
    const targetOffsetPct = ((targetY - padTop) / innerHeight) * 100
    ...
    const labelStep = days.length > 7 ? Math.ceil(days.length / 7) : 1

    const yTicks = yAxisTicks(yMax, yAxisStep)
  ...
  }, [days, max, target, chartHeight, padTop, padBottom, todayKey, fixedScale, yAxisStep])
  ...
  return (
    <div className="rounded-xl border border-slate-200/90 bg-white/80 shadow-sm dark:border-slate-700/80 dark:bg-slate-900/50">
      <div
        ...
        aria-label={needsHorizontalScroll ? 'XP-Verlauf, horizontal scrollen für ältere Tage' : undefined}
      >
        <svg
        ...
        >
          ...
          {layout.yTicks.map((tick) => {
            const y = padTop + layout.innerHeight * (1 - tick.fraction)
            return (
              <g key={tick.value}>
                <line
                  x1={layout.axisX}
          ...
          {layout.startMarkerX != null ? (
            <g>
              ...
              <text
                ...
                y={layout.baselineY + START_LABEL_OFFSET}
                textAnchor="middle"
          ...
          {layout.areaPath ? <path d={layout.areaPath} fill={`url(#${areaGradientId})`} stroke="none" /> : null}
          ...
          {layout.chartPoints.map((point) => {
            ...
            const radius = point.isToday ? 5.5 : 4
            const fill =
              point.isToday && point.xp >= target
                ? '#047857'
                : point.isToday
                  ? '#d97706'
                  : point.xp >= target
            ...
            return (
              <g key={point.date ?? point.role}>
                ...
                <circle
                  ...
                  fill={fill}
                  stroke="#fff"
                  strokeWidth={point.isToday ? 2 : 1.5}
                  className="dark:stroke-slate-900"
                />
              </g>
            )
          ...
          {layout.chartPoints.map((point, index) => {
            if (point.role === 'origin' || point.role === 'start') return null
            const dayIndex = index - 1
            if (dayIndex % layout.labelStep !== 0 && dayIndex !== days.length - 1 && !point.isToday) return null
            if (!point.date) return null
            return (
              <text
                key={`${point.date}-label`}
                x={point.x}
                y={layout.baselineY + DATE_LABEL_OFFSET}
                textAnchor="middle"
                className={`text-[8px] ${
                  point.isToday
                    ? 'fill-sky-700 font-bold dark:fill-sky-300'
